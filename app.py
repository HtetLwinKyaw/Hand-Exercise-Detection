...
# Function to predict exercise and provide feedback
...
def main():
    ...
    st.sidebar.markdown("This app uses MediaPipe and a machine learning model to detect hand exercises.")
    ...
    feedback_container = st.empty()  # Container for feedback display
    ...
    if run:
        ...
        while cap.isOpened():
            ...
            # Display the annotated frame on Streamlit
            FRAME_WINDOW.image(cv2.cvtColor(annotated_frame, cv2.COLOR_BGR2RGB))
            
            # Display the feedback and exercise name dynamically
            feedback_text = ""
            if predictions:
                for i, prediction in enumerate(predictions):
                    feedback_function_name = f'provide_feedback_{prediction.replace("-", "_")}'
                    ...
                    # Format exercise name and feedback
            ...
            # Update the Streamlit container with the latest feedback and exercise details
